...
namespace LOS_Security
{
    public static class LosSecurity
    {
        ...
        {
            if (unsafeText == null)
                throw new ArgumentNullException("unsafeText");

            unsafe
            {
                fixed (char* passwordChars = unsafeText)
                {
                    var securePassword = new SecureString(passwordChars, unsafeText.Length);
